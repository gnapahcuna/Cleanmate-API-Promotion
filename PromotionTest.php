...
foreach($_POST as $object) {
	...
	$stmt_ser="select top 1 (case when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL then $price
...
where p.IsActive=2 AND BranchGroupID=$BranchGroupID AND '".$Date."' between EffectiveDate and ExpirationDate
Order by Amount Desc";
	...
	AND ConditionType =2 AND ConditionAmount IS NULL AND $Amount>=ConditionPrice then 1
	when ServiceType =$ServiceType AND CategoryID =$CategoryID AND ProductID IS NULL
	AND ConditionType =2 AND ConditionAmount IS NULL AND $Amount>=ConditionPrice then 1
  ...
  when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND ConditionType=1 AND
	BranchID=$BranchID AND CustomerID=$CustomerID AND MemberTypeID=$MemberID AND
	ConditionAmount<=$Total AND DiscountType=2 AND DiscountPriceType=2
	AND ((case when $Total>ConditionAmount  then
	(ConditionAmount+($Total-ConditionAmount))-(($Total/ConditionAmount)-1)
	else ConditionAmount end) =$i OR
  ...
  when $ServiceType=ServiceType AND CategoryID =$CategoryID AND ProductID IS NULL AND ConditionType=1 AND
	BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID=$MemberID AND
	ConditionAmount<=$Total AND DiscountType=2 AND DiscountPriceType=2
	AND ((case when $Total>ConditionAmount  then
	(ConditionAmount+($Total-ConditionAmount))-(($Total/ConditionAmount)-1)
	else ConditionAmount end) =$i OR
	...
	when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND ConditionType=1 AND
	BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND
	ConditionAmount<=(case when ServiceType=1 AND $ServiceType=1  then $Total
  when ServiceType=2 AND $ServiceType=2 then $Total1
  when ServiceType=3 AND $ServiceType=2 then $Total2 else 0 end) AND DiscountType=1
  ...
  when $ServiceType=ServiceType AND CategoryID=$CustomerID AND ProductID IS NULL AND ConditionType=1 AND
	BranchID=$BranchID AND CustomerID=$CustomerID AND MemberTypeID=$MemberID AND
	ConditionAmount<=(case when ServiceType=1 AND $ServiceType=1  then $Total
  when ServiceType=2 AND $ServiceType=2 then $Total1
  when ServiceType=3 AND $ServiceType=2 then $Total2 else 0 end) AND DiscountType=1
  ...
  when $ServiceType=ServiceType AND CategoryID=$CategoryID AND ProductID=$ProductID AND ConditionType=1 AND
  BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND
  ConditionAmount<=(case when ServiceType=1 AND $ServiceType=1  then $Total
when ServiceType=2 AND $ServiceType=2 then $Total1
when ServiceType=3 AND $ServiceType=2 then $Total2 else 0 end) AND DiscountType=1
...
when $ServiceType=ServiceType AND CategoryID=$CategoryID AND ProductID=$ProductID AND ConditionType=1 AND
  ...
  when ServiceType=3 AND $ServiceType=2 then $Total2 else 0 end) AND DiscountType=1
	...
	when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID =$BranchID AND CustomerID IS NULL AND MemberTypeID IS NULL  AND ConditionType IS NULL AND
  ...
  when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID=$MemberID  AND ConditionType IS NULL AND
  ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST($price as float)*DiscountRate)/100
  ...
  when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID IS NULL AND ConditionType IS NULL AND
	...
	when $ServiceType=ServiceType AND CategoryID =$CategoryID AND ProductID =$ProductID AND BranchID =$BranchID AND CustomerID=$CustomerID AND MemberTypeID =$MemberID AND ConditionType IS NULL AND
		ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST($price as float)*DiscountRate)/100
	...
	when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType =2 AND
	...
	when $ServiceType=ServiceType AND CategoryID =$CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType =2 AND
	ConditionAmount IS NULL AND $Amount>=ConditionPrice AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

	when $ServiceType=ServiceType AND CategoryID =$CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType =2 AND
  ConditionAmount IS NULL AND $Amount>=ConditionPrice AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST(ConditionPrice as float)*DiscountRate)/100

  when $ServiceType=ServiceType AND CategoryID=$CategoryID AND ProductID IS NULL AND BranchID=$BranchID AND CustomerID=$CustomerID AND MemberTypeID=$MemberID AND ConditionType=2 AND
	ConditionAmount IS NULL AND $Amount>=ConditionPrice AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

	when $ServiceType=ServiceType AND CategoryID=$CategoryID AND ProductID IS NULL AND BranchID=$BranchID AND CustomerID=$CustomerID AND MemberTypeID=$MemberID AND ConditionType=2 AND
  ConditionAmount IS NULL AND $Amount>=ConditionPrice AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST(ConditionPrice as float)*DiscountRate)/100

  when $ServiceType=ServiceType AND CategoryID =$CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID =$MemberID AND ConditionType =2 AND
	ConditionAmount IS NULL AND $Amount>=ConditionPrice AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

	when $ServiceType=ServiceType AND CategoryID =$CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID =$MemberID AND ConditionType =2 AND
	ConditionAmount IS NULL AND $Amount>=ConditionPrice AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST(ConditionPrice as float)*DiscountRate)/100

  when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID=$BranchID AND CustomerID =$CustomerID AND MemberTypeID =$MemberID AND ConditionType =1 AND
  ...
  when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID IS NULL AND CustomerID =$CustomerID AND MemberTypeID =$MemberID AND ConditionType =1 AND
	$i=ConditionAmount AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then ($i/ConditionAmount)*DiscountPrice

	when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID IS NULL AND CustomerID =$CustomerID AND MemberTypeID =$MemberID AND ConditionType =1 AND
	$i=ConditionAmount AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then ($i/ConditionAmount)*((CAST(ConditionPrice as float)*DiscountRate)/100)

	when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType =1 AND
	$i=ConditionAmount AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then ($i/ConditionAmount)*DiscountPrice

	when $ServiceType=ServiceType AND CategoryID IS NULL AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType =1 AND
	$i=ConditionAmount AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then ($i/ConditionAmount)*((CAST(ConditionPrice as float)*DiscountRate)/100)

	when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID =$ProductID AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType IS NULL AND
	ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

	when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID =$ProductID AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType IS NULL AND
  ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST($price as float)*DiscountRate)/100

  when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID =$ProductID AND BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID=$MemberID AND ConditionType IS NULL AND
	ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

	when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID =$ProductID AND BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID=$MemberID AND ConditionType IS NULL AND
	ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST($price as float)*DiscountRate)/100

	when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType IS NULL AND
	ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

	when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType IS NULL AND
	ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST($price as float)*DiscountRate)/100

	when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID IS NULL AND BranchID =$BranchID AND CustomerID=$CustomerID AND MemberTypeID=$MemberID  AND ConditionType IS NULL AND
  ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

  when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID IS NULL AND BranchID =$BranchID AND CustomerID=$CustomerID AND MemberTypeID=$MemberID  AND ConditionType IS NULL AND
  ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST($price as float)*DiscountRate)/100

  when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID=$MemberID  AND ConditionType IS NULL AND
  ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

  when $ServiceType=ServiceType AND CategoryID= $CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID=$CustomerID AND MemberTypeID=$MemberID  AND ConditionType IS NULL AND
	ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST($price as float)*DiscountRate)/100

	when ServiceType IS NULL AND CategoryID = $CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType IS NULL AND
	...
	when ServiceType IS NULL AND CategoryID IS NULL AND ProductID = $ProductID AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType IS NULL AND
	ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=2 then (CAST($price as float)*DiscountRate)/100

	when ServiceType IS NULL AND CategoryID IS NULL AND ProductID = $ProductID AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType IS NULL AND
	ConditionAmount IS NULL AND ConditionPrice IS NULL AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice

	when ServiceType IS NULL AND CategoryID IS NULL AND ProductID IS NULL AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType =2 AND
	ConditionAmount IS NULL AND $Amount>=ConditionPrice AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice
  ...
  when ServiceType=$ServiceType AND CategoryID=$CategoryID AND ProductID IS NULL AND BranchID IS NULL AND CustomerID =$CustomerID AND MemberTypeID=$MemberID AND ConditionType =2 AND
    ...
    when ServiceType=$ServiceType AND CategoryID=$CategoryID AND ProductID=$ProductID AND BranchID IS NULL AND CustomerID IS NULL AND MemberTypeID IS NULL AND ConditionType=2 AND
	ConditionAmount IS NULL AND $Amount>=ConditionPrice AND DiscountType=1 AND DiscountProductNo IS NULL AND DiscountPriceType=1 then DiscountPrice
...
foreach ($array as $key => $value){
	...
	for($i=0;$i<count($arr_promo_id);$i++){
		if($value==$arr_promo_id[$i]){
            $total+=$arr_promo_price[$i];
			//array_push($object_array,array('PromotionID' => $value, 'Price' => $total, 'Description' => iconv("Windows-874", "utf-8", $arr_promo_detail[$i])));
